...
A label-less email with patch content (inline diff or =.patch=
attachment) is also recognised as a patch, but only in a new thread,
with no =In-Reply-To=. To promote a reply as patch, you need to change
the subject and add the =[PATCH]= prefix or to attach a proper patch,
real =git format-patch= output.
...
Replying to an open bug or request with patch content (inline diff
or =.patch=/=.diff= attachment) credits you as =Acked= + =Owned= on
the *nearest* parent report.  An existing explicit setter is
preserved; opt out with =Not acked.= / =Not owned.=.

When the reply also produces a separate =:patch= report (labelled
=Re: [PATCH]=, or a real =git format-patch= attachment), the patch is
...
A bare diff in a thread that does not become a patch report does not
propagate -- the parent must still be closed manually.
...
The reply still needs to qualify as a patch report (=Re: [PATCH]= with
content, or a real =git format-patch= attachment in the reply) -- a
bare =Re:= with a quoted diff does not create a new report and so
