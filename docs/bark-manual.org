...
format and rules.  At startup, =bark= materialises the tenure history
by syncing at each period boundary, and resolves the effective config
for every email based on its date.
...
=bb test-config= validates the =:periods= on each source: contiguity,
