...
mark (=.= =,= =;= =:=). For example, replying to a bug report with a line
starting with =Confirmed.= marks the bug as acked.
...
them globally or per source in =config.edn= via =:labels=. Specify only
the types you want to change; unspecified types keep their defaults.
