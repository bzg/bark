...
- *JVM* (=clojure -M:run=) --- Connects to IMAP, fetches new emails
  since the last run, digests them into reports, applies commands and
  roles, expires stale reports, then exits.  Use =--watch= to keep the
  process running with IMAP IDLE.  All database writes happen here.
