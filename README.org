...
A source has no distinct admin: the only privileged role is
=maintainer=. Maintainers are seeded from the per-source =:maintainers=
vector; the *first entry* is the *lead maintainer*.
