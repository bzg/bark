...
- *JVM* (=clojure -M:run=) :: Connects to an IMAP server or a local
  ...
  reports, then exits. Use =--watch= to keep the process running (IMAP
  IDLE for IMAP, filesystem events for Maildir). All database writes
  happen here.
- *bb export* :: Exports reports as JSON, RSS, Org, ICS (calendar), and
  generates HTML and stats pages. Read-only (no database writes).
- *bb notify* :: Sends notification emails to maintainers. Read-only (no
  database writes).
