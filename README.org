...
BARK is an email-driven report tracker - see the [[docs/bark-manual.org#rationale][rationale]].
...
- *bb export* --- Exports reports as JSON, RSS, Org, and generates HTML
  and stats pages. Read-only (no database writes).
...
| =bark-export.clj=     | Report export (JSON, RSS, Org, patches) |
...
#+begin_example
bb export [json|rss|org|html|stats|all]   Export reports (default: all)
...
| =:bark-path=             | no       | Base URL where this source's public directory is served              |
