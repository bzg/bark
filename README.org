...
- *JVM* (=clojure -M:run=) --- Connects to an IMAP server or a local
  Maildir directory, fetches new emails since the last run, digests
  them into reports, applies commands and roles, expires stale
  reports, then exits.  Use =--watch= to keep the process running
  (IMAP IDLE for IMAP, filesystem events for Maildir).  All database
  writes happen here.
- *bb export* --- Exports reports as JSON, RSS, Org, ICS (calendar),
  and generates HTML and stats pages. Read-only (no database writes).
- *bb notify* --- Sends notification emails to maintainers.
...
bb rebuild-history [--history PATH] [--fresh] [--dry-run]
                                          Replay a mail archive across multiple
                                          era configs (see bark-manual.org)
...
|----------------------+---------+----------------------------------------------------------------------------|
| =:fetch=               | =50=      | First-run fetch: count, duration (="30d"=), ISO date, or =:since=/=:until= map |
...
=[from, to)= interval recording when someone held maintainer status.
The full tenure history (active and closed) is exported in
=meta.json= and rendered in =docs.html=, so a new deployment can mirror
command processing on archived emails even when maintainers have
changed over time.

The per-maintainer =:since= field is optional; omitting it means "active
since the beginning of time" (the usual default for a fresh deploy).
...
- =Users= : sysadmins who deploy, configure, and operate BARK instances
