...
A single mail source (IMAP or Maildir) shared by all sources.
...
through the source's public channel.  Anyone running BARK on the
same source then sees the same state.
...
status.  The full history (active and closed) is exported in
=meta.json= and rendered in =docs.html=, so a fresh deployment
...
Applied at export time only (the DB is untouched) to maintainers,
authors, and owners.  Unlisted emails keep their last observed
=From:= name.
...
top level or per source (per-source wins).  Only the types you
