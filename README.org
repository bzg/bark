...
#+begin_example
bb export [json|rss|org|html|stats|patches|text|events|all]
...
*Direct emails from maintainers:* when a maintainer sends directly to
the BARK inbox (bypassing the list or alias), they must signal the
target source with either a =X-Bark-Source: source-name= header or a
=[bark:source-name]= prefix in the subject.  Non-maintainer direct
emails to =:list= or =:alias= sources are ignored.
...
- Send a [[mailto:~bzg/barkyard@lists.sr.ht?subject=\[BUG\] bark: <SHORT EXPLICIT BUG DESCRIPTION>][bug report]]
- Send a [[mailto:~bzg/barkyard@lists.sr.ht?subject=\[PATCH\] bark: <COMMIT SUMMARY>][patch]] (please use =git format-patch=)
- Send a [[mailto:~bzg/barkyard@lists.sr.ht?subject=\[FR\] bark: <FEATURE REQUEST>][feature request]]
- Share any [[mailto:~bzg/barkyard@lists.sr.ht?subject=bark: <ANYTHING>][other question or idea]]
