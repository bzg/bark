...
| Namespace     | Role                                                |
|---------------+-----------------------------------------------------|
| =bark.main=     | Daemon entry point (IMAP IDLE loop)                 |
| =bark.db=       | Datalevin connection, schema, queries               |
| =bark.ingest=   | Email parsing and storage                           |
| =bark.pipeline= | Atomic store-then-digest per email                  |
